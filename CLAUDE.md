...
**⏳ EN ATTENTE — à faire par J-C (prévu demain)** : **redéployer le coffre-fort**. `gas/api.gs` a été refait en **mode « action »** (voir Vitesse). Coller le nouveau contenu dans le script `api`, puis **Déployer → Gérer les déploiements → crayon ✏️ → Nouvelle version** (GARDER la même URL, jamais « Nouveau déploiement »). Tant que ce n'est pas fait, l'app marche mais en **repli lent** (le client a un repli automatique). **Màj 2026-09-20** : `api.gs` gère la **marque/format par entrée** (sur STOCK, plus sur le produit), renvoie les **variantes par produit** (suggestions), et est **à l'épreuve du reclic** (jeton `opId` vérifié avant d'écrire). Colonnes STOCK **F Marque · G Format · H OpId** = **déjà ajoutées par J-C**. **Màj (auto-remplissage)** : `variantesStock()` renvoie aussi, par produit, les **emplacements habituels dans l'ordre d'apparition** (1er entré = #1) + la **dernière marque/format** → à la re-sélection d'un produit connu, la fiche pré-remplit marque/format et crée **un endroit par emplacement habituel** (quantité vide; un endroit à 0 n'est pas créé). Reste : **redéployer `api.gs`** (le même redéploiement en attente couvre tout ça). **Màj (scan, 2026-09-20)** : le **code-barres** vit sur **STOCK col I** (pas de table CODES) — **J-C doit ajouter la colonne I « CodeBarres »**. `api.gs` écrit le code à l'entrée et renvoie un index **`codes` = { codeBarres: produitId }** (reconnaître un produit déjà à nous au scan). Tant que non redéployé : l'**écriture** du code marche en repli, mais la **reconnaissance** au re-scan attend le redéploiement. Scan branché : `JS/scan.js` (moteur WASM zxing-wasm) → Open Food Facts (identité) → fiche; voir `docs/RdG-sources-donnees.md`.
