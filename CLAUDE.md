...
- ⚠️ **Deux conversations sur le même dépôt (depuis 2026-09-20, partage précisé le 2026-09-22)** : une travaille les **fonctions** (`JS/`, `gas/`), l'autre le **look** (**`CSS/rdg.css` ET `rdg.html`** — le HTML n'avait pas de propriétaire, c'est réglé : il suit le look). Si la conversation des fonctions a besoin d'un élément dans la page ou d'une classe, **elle le demande au lieu de l'écrire**. Règles : **`git pull` AVANT chaque push**; ne pas écrire dans le fichier de l'autre; **signaler à J-C tout ajout de CSS** au lieu d'en inventer. ⚠️ **J-C n'est pas un facteur** : ne pas lui faire porter des messages d'une conversation à l'autre plus longtemps qu'il ne faut — poser la règle dans ce fichier, qui les atteint toutes les deux, et le libérer. ⚠️ Les deux poussent sur **`main`** — c'est ce que GitHub Pages sert, donc c'est la seule branche où J-C peut tester. (Leçon apprise : un `CLAUDE.md` poussé sur une branche séparée **n'atteint jamais l'autre conversation**.)
- **Le look de `rdg.html` (fait 2026-09-21, tout dans `CSS/rdg.css`)** :
  ...
  - **`accordeon-bloc` et `accordeon-item-saisie` (posées le 2026-09-22)** : deux classes de la famille des accordéons, demandées par la conversation des fonctions pour sortir les deux derniers `style=` que `entree.js` écrit dans `htmlMeuble()`. ⚠️ **Elles peuvent paraître inutilisées tant que son chantier n'est pas poussé — ne pas les effacer.** (Leçon du même jour : chercher `style=` dans les `.js` aussi, pas seulement `.style.` — le JS en écrit dans le HTML qu'il fabrique.)
