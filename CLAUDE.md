...
**À suivre (parkings)** : rangement des « en transit »; le scan (rafale d'arrivée d'épicerie); brancher les 4 blocs + navigation accueil↔fiche; contenu des accordéons (manque, péremption, spéciaux, listes = pts 5/6/12); durée de vie sur les sous-catégories; page de gestion des listes de base; **page unique / SPA (panneaux au lieu de `.html` séparés) — reco du dev Dionysos, vaut pour le projet `rdg.html` SEUL. ⚠️ `index.html` et `rdg.html` = 2 projets SÉPARÉS, jamais les fusionner.**
